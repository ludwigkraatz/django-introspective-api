...
class IntegrationTestPagination(TestCase):
    ...
    def test_get_paginated_root_view_in_header(self):
        ...
        self.assertEquals(response['Accept-Ranges'], ','.join(RootView.settings.PAGINATION_RANGE_HEADER_TOKEN))
        ...
        request.META[u'HTTP_RANGE'] = '%(token)s=%(start)s-%(end)s' % {
            'token': RootView.settings.PAGINATION_RANGE_HEADER_TOKEN[0],
            'start': 0,
            'end': 9
            }
        response = self.view(request).render()
        ...
        request = factory.get('/', HTTP_RANGE='%s=10-19' % RootView.settings.PAGINATION_RANGE_HEADER_TOKEN[0])
        response = self.view(request).render()
        self.assertEquals(response.status_code, status.HTTP_206_PARTIAL_CONTENT)
        self.assertEquals(response['Content-Range'], '%s 10-19/26' % RootView.settings.PAGINATION_RANGE_HEADER_TOKEN[0])
        self.assertEquals(response.data, self.data[10:20])
        self.assertNotIn('Link', response)
        self.assertEquals(response['Accept-Ranges'], RootView.settings.PAGINATION_RANGE_HEADER_TOKEN[0])
        request = factory.get('/', HTTP_RANGE='%s=10-' % RootView.settings.PAGINATION_RANGE_HEADER_TOKEN[0])
        response = self.view(request).render()
        self.assertEquals(response.status_code, status.HTTP_206_PARTIAL_CONTENT)
        self.assertEquals(response['Content-Range'], '%s 10-25/26' % RootView.settings.PAGINATION_RANGE_HEADER_TOKEN[0])
        self.assertEquals(response.data, self.data[10:])
        self.assertNotIn('Link', response)
        self.assertEquals(response['Accept-Ranges'], RootView.settings.PAGINATION_RANGE_HEADER_TOKEN[0])
        request = factory.get('/', HTTP_RANGE='%s=-9' % RootView.settings.PAGINATION_RANGE_HEADER_TOKEN[0])
        response = self.view(request).render()
        self.assertEquals(response.status_code, status.HTTP_206_PARTIAL_CONTENT)
        self.assertEquals(response['Content-Range'], '%s 17-25/26' % RootView.settings.PAGINATION_RANGE_HEADER_TOKEN[0])
